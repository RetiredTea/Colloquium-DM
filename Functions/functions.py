...
def GCF_NN_N(num1: NaturalNumber, num2: NaturalNumber):
    def swap(a: NaturalNumber, b: NaturalNumber): # меняем, максимальное и минимальное местами, если это нужно
        if COM_NN_D(a,b) == 2:
            return a, b
        c = a
        a = b
        b = c
        return a, b
    while NZER_N_B(num1) == False and NZER_N_B(num2) == False:  #Цикл Пока никакое не ноль
        num1, num2 = swap(num1,num2) # меняем местами если это нужно
        num1 = MOD_NN_N(num1,num2) # меняем местами если это нужно
    if(int(num2) == 0 and int(num1) != 0):
        return num1
    elif(int(num2) == 0 and int(num1) == 0):
        return NaturalNumber("1") #находим остаток от деления
    return num2
...
def LCM_NN_N(num1: NaturalNumber, num2: NaturalNumber):
    return DIV_NN_N(MUL_NN_N(num1,num2), GCF_NN_N(num1, num2)) # находит частное от деления произведения двух чисел на их НОД
...
def ADD_QQ_Q (frac1: RationalNumber, frac2: RationalNumber):
    denominator = (LCM_NN_N(frac1.denominator,frac2.denominator)) # находим НОК знаменателей
    numerator = ADD_ZZ_Z(MUL_ZZ_Z(frac1.numerator, TRANS_N_Z((DIV_NN_N(denominator, frac1.denominator)))),\
                        MUL_ZZ_Z(frac2.numerator, TRANS_N_Z(DIV_NN_N(denominator, frac2.denominator)))) # вычисляем числитель новой дроби с импользований функций сложения
...
def SUB_QQ_Q(frac1: RationalNumber, frac2: RationalNumber):
    ...
    numerator = SUB_ZZ_Z(MUL_ZZ_Z(frac1.numerator, TRANS_N_Z((DIV_NN_N(denominator, frac1.denominator)))),\
                        MUL_ZZ_Z(frac2.numerator, TRANS_N_Z(DIV_NN_N(denominator, frac2.denominator))))# вычисляем числитель новой дроби с импользований функций разности
    frac_sum = RationalNumber(numerator, denominator)# целых чисел, числитель каждой дроби вычисляется как умножение старого числителя на частное нового знаминателя и старого
    return(frac_sum)
...
def DER_P_P(pln: Polynomial):
    ...
    temp = polyn.head
    while temp is not None:
        if str(temp.deg) != '1' and str(temp.deg) != '0': # если степень не 1 и не 0
            temp.val = MUL_QQ_Q(temp.val, TRANS_Z_Q(TRANS_N_Z(temp.deg)))
            temp.deg = SUB_NN_N(temp.deg, NaturalNumber('1')) # вычитаем из степени 1
        elif str(temp.deg) == '1':# если степень равна 1, то степень = 0
            temp.deg = '0'

        elif str(temp.deg) == '0': # если степень равна 0, то удаляем звено списка
            if (temp.prev != None):
                temp = temp.prev
                temp.next = None
            else:
