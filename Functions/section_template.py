            ...
            if int(func_number) == 35:
                ...
                for i in range(len(args)):
                    try:
                        ...
                        self.error_label.config(text=f"Ошибка: некорректный ввод для '{args[i]}'")
                        return
                args = res
                    ...
                    try:
                        if 29 <= int(func_number) <= 45:
                            ...
                            if len(str(result_text)) > 50:
                                ...
                                os.startfile(file_path)  # Открываем файл с результатом
                                self.result_label.config(text="Результат сохранён в result.txt")
                            else:
